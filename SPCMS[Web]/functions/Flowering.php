...
if (isset($_POST['id']))
{
  echo json_encode(FloweringData($_POST['id']));
}


function FloweringData($_id)
{
  $output = DB::getInstance()->get('flowering_inspection', array('inspection_id' ,'=' , $_id));
  $results = array();
  ...
  if($output)
  {
      foreach ($output->results() as $output)
      {
        ...
        $results["isolation_maintain"] = $output->isolation_maintain;
        $results["off_type_removal"] = $output->off_type_removal;
