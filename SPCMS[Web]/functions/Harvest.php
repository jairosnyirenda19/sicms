...
if (isset($_POST['id']))
{
  echo json_encode(HarvestData($_POST['id']));
}


function HarvestData($_id)
{
  $output = DB::getInstance()->get('harvest_inspection', array('inspection_id' ,'=' , $_id));
  $results = array();
  ...
  if($output)
  {
      foreach ($output->results() as $output)
      {
        ...
        $results["maturity"] = $output->maturity;
