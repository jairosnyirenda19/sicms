...
if (isset($_POST['id']))
{
  echo json_encode(PostFloweringData($_POST['id']));
}


function PostFloweringData($_id)
{
  $output = DB::getInstance()->get('post_flowering_inspection', array('inspection_id' ,'=' , $_id));
  $results = array();
  ...
  if($output)
  {
      foreach ($output->results() as $output)
      {
        ...
        $results["issues_taken_care"] = $output->issues_taken_care;
